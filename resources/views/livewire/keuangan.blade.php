<div class="space-y-8 pb-10" x-data="{ 
    ...
    <div x-show="activeTab === 'goals'" x-transition.opacity.duration.300ms style="display: none;">
        <div class="space-y-6">
            ...
            @if(count($goals) === 0)
                <div class="bg-white rounded-[2rem] border border-slate-100 p-10 text-center">
                    <div class="flex flex-col items-center justify-center text-slate-400">
                        <x-lucide-target class="w-12 h-12 mb-3 text-slate-200" />
                        <p class="font-bold text-slate-500">Belum ada Goals</p>
                        <p class="text-sm">Rencanakan target tabungan barumu di sini.</p>
                    </div>
                </div>
            @else
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    @foreach($goals as $goal)
                        ...
                        <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-6 relative group hover:shadow-lg transition-all">
                            <div class="absolute top-4 right-4 flex gap-1 z-10">
                                ...
                                <button wire:click="confirmDelete('goal', {{ $goal->id }}, '{{ addslashes($goal->title) }}')" class="p-1.5 text-slate-400 hover:text-rose-500 hover:bg-slate-50 rounded-lg transition-colors">
                                    <x-lucide-trash-2 class="w-4 h-4" />
                                </button>
                            </div>
                            ...
                            <button wire:click="manageGoal({{ $goal->id }})" class="w-full mt-2 py-3 bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2">
    ...
    @if($showAddGoalModal)
    <div class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-navy-950/40 backdrop-blur-sm" wire:click="$set('showAddGoalModal', false)"></div>
        <div class="bg-white translate-z-0 w-full max-w-xl rounded-[2.5rem] shadow-2xl relative z-10 overflow-hidden flex flex-col">
            <!-- Header -->
            <div class="bg-emerald-500 px-6 py-5 text-white relative flex items-center justify-between">
                <div class="flex items-center gap-3">
                ...
                <button wire:click="$set('showAddGoalModal', false)" class="p-1.5 bg-white/15 hover:bg-white/25 rounded-lg transition-colors">
                    <x-lucide-x class="w-[18px] h-[18px]" />
                </button>
            </div>
            ...
            <form wire:submit="saveGoal" class="flex flex-col">
                ...
                <!-- Footer -->
                <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-3 rounded-b-[2.5rem]">
                    <button type="button" wire:click="$set('showAddGoalModal', false)" class="px-5 py-2.5 rounded-xl text-xs font-bold text-slate-500 hover:bg-slate-200 transition-all uppercase tracking-widest active:scale-95">
                        Batal
                    </button>
                    <button type="submit" class="bg-emerald-500 hover:bg-emerald-600 text-white px-7 py-2.5 rounded-xl font-black text-xs shadow-xl shadow-emerald-500/10 active:scale-95 transition-all flex items-center gap-2 uppercase tracking-widest">
    ...
    @if($showManageGoalModal)
    <div class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-navy-950/40 backdrop-blur-sm" wire:click="$set('showManageGoalModal', false)"></div>
        <div class="bg-white translate-z-0 w-full max-w-xl rounded-[2.5rem] shadow-2xl relative z-10 overflow-hidden flex flex-col">
            <!-- Header -->
            <div class="bg-emerald-500 px-6 py-5 text-white relative flex items-center justify-between">
                <div class="flex items-center gap-3">
                ...
                <button wire:click="$set('showManageGoalModal', false)" class="p-1.5 bg-white/15 hover:bg-white/25 rounded-lg transition-colors">
                    <x-lucide-x class="w-[18px] h-[18px]" />
                </button>
            </div>

            <!-- Form Content -->
            <form wire:submit="saveGoalSavings" class="flex flex-col">
                <div class="p-6 space-y-5 overflow-hidden">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <!-- Nominal Setoran -->
                        <div class="space-y-1.5">
                            ...
                            <div class="relative group">
                                <span class="absolute left-3.5 top-1/2 -translate-y-1/2 font-black text-slate-400 group-focus-within:text-emerald-500 text-xs">Rp</span>
                                <div x-data="{ raw: @entangle('goalAddAmount'), formatted: '' }" x-init="formatted = raw ? new Intl.NumberFormat('id-ID').format(raw) : ''; $watch('raw', val => { if(!val) formatted = ''; else if(val != String(formatted).replace(/\D/g, '')) formatted = new Intl.NumberFormat('id-ID').format(val); })">
                                    <input type="text" x-model="formatted" @input="let val = String(formatted).replace(/\D/g, ''); raw = val ? parseInt(val) : null; formatted = val ? new Intl.NumberFormat('id-ID').format(val) : '';" placeholder="0" class="w-full bg-slate-50 border border-slate-100 rounded-2xl py-2.5 pl-[2.25rem] pr-4 text-xs font-bold text-navy-900 focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500/50 focus:bg-white outline-none transition-all" required />
                                </div>
                            </div>
                            @error('goalAddAmount') <span class="text-rose-500 text-[10px] ml-1">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <div class="relative group">
                                ...
                                <select wire:model="goalAddWallet" class="w-full bg-slate-50 border border-slate-100 rounded-2xl py-2.5 pl-10 pr-4 text-xs font-bold text-navy-900 focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500/50 focus:bg-white outline-none transition-all appearance-none cursor-pointer" required>
                                    <option value="">Pilih wallet</option>
                                    @foreach($wallets as $wallet)
                            ...
                            @error('goalAddWallet') <span class="text-rose-500 text-[10px] ml-1">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="relative group">
                            <x-lucide-calendar class="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-emerald-500 transition-colors w-4 h-4" />
                            <input type="date" wire:model="goalAddDate" class="w-full bg-slate-50 border border-slate-100 rounded-2xl py-2.5 pl-10 pr-4 text-xs font-bold text-navy-900 focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500/50 focus:bg-white outline-none transition-all" required />
                        </div>
                        @error('goalAddDate') <span class="text-rose-500 text-[10px] ml-1">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="relative group">
                            <x-lucide-align-left class="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-emerald-500 transition-colors w-4 h-4" />
                            <input type="text" wire:model="goalAddNotes" placeholder="Contoh: Setoran awal bulan" class="w-full bg-slate-50 border border-slate-100 rounded-2xl py-2.5 pl-10 pr-4 text-xs font-bold text-navy-900 focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500/50 focus:bg-white outline-none transition-all" />
                        </div>
                    </div>
                </div>
                ...
                <!-- Footer -->
                <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-3 rounded-b-[2.5rem]">
                    <button type="button" wire:click="$set('showManageGoalModal', false)" class="px-5 py-2.5 rounded-xl text-xs font-bold text-slate-500 hover:bg-slate-200 transition-all uppercase tracking-widest active:scale-95">
                        Batal
                    </button>
                    <button type="submit" class="bg-emerald-500 hover:bg-emerald-600 text-white px-7 py-2.5 rounded-xl font-black text-xs shadow-xl shadow-emerald-500/10 active:scale-95 transition-all flex items-center gap-2 uppercase tracking-widest">
